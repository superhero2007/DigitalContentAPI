...
namespace Digital_Content.DigitalContent.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : DigitalContentServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("Digital_Content.DigitalContent", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
