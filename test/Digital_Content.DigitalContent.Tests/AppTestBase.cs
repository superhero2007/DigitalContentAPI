...
using Digital_Content.DigitalContent.Tests.TestDatas;
...
namespace Digital_Content.DigitalContent.Tests
{
    ...
    /// Provides methods to easily work with <see cref="DigitalContentDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<DigitalContentTestModule>
    {
        ...
        {
            void NormalizeDbContext(DigitalContentDbContext context)
            {
        ...
        protected void UsingDbContext(Action<DigitalContentDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<DigitalContentDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<DigitalContentDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<DigitalContentDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<DigitalContentDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DigitalContentDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<DigitalContentDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DigitalContentDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<DigitalContentDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<DigitalContentDbContext, Task<T>> func)
        {
            T result;
